...
class SearchService {
  ...
  async getLocations(searchRequest, targetLanguage) {
    try {
      let searchResponses = await bppSearchService.getLocations(searchRequest);
      if (targetLanguage) {
        //translate data
        return await translateObject(
          searchResponses,
  ...
  async getAttributesValues(searchRequest) {
    try {
      return await bppSearchService.getAttributesValues(searchRequest);
    } catch (err) {
      throw err;
    }
  }
  ...
  async getCustomMenus(searchRequest) {
    try {
      return await bppSearchService.getCustomMenus(searchRequest);
    } catch (err) {
      throw err;
    }
  }
  ...
  validateSchedule(searchObj) {
    ...
    if (searchObj.location_details.time) {
      ...
      if (searchObj.location_details.time.days) {
        let opendays = searchObj.location_details.time.days
          .split(",")
          .map(Number);
      ...
      if (searchObj.location_details.time.range) {
        //check for range
        ...
        if (searchObj.location_details.time.schedule.frequency) {
          ...
          let period = createPeriod({
            start: storeOpenTime,
            duration: searchObj.location_details.time.schedule.frequency,
            recurrence: 1,
          });
  ...
  async getFilterParams(query) {
    try {
      ...
      return {
        ...
        fulfillment: Array.from(fulfillmentList, ([id, value]) => ({
          id,
          value,
        })),
        minPrice: minPrice,
